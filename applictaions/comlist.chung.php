...
<body>
    <?php
      ...
      $query = ("SELECT * FROM `彰化縣托嬰中心評價` order by 機構名稱" );
      ...
      while($row = mysqli_fetch_array($result)) {
        ...
        echo "<td>" . $row['機構名稱'] . "</td>";
        echo "<td>" . $row['評價'] . "</td>";
        echo "<td><form class='form-horizontal' method='post' action='com_update_chung.php'> 
              <input name='comment_id' type='hidden' value='".$row['comment_id']."';>
              <input type='submit' class='btn btn-primary' name='update' value='update'>
              </form></td>";
        echo "<td><form class='form-horizontal' method='post' action='comlist.chung.php'>
        <input name='comment_id' type='hidden' value='".$row['comment_id']."';>
      ...
      if(isset($_POST['delete'])) {
        ...
        $sql = mysqli_query($connect,"DELETE FROM 彰化縣托嬰中心評價 WHERE comment_id='$comment_id'");
        if(!$sql) {
        ...
        $comment_id = mysqli_real_escape_string($connect, $_POST['comment_id']);
        header("Location: com_update_chung.php?comment_id=" . $comment_id);
      }
    ?>
</fieldset>
</form>
